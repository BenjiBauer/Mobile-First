...
function Update () {
	...
	if(bullet1.active==true&&GameSystemFkt.bulletIsFlying==false){
		...
		CamTrans.position = Vector3.MoveTowards(CamTrans.position,bullet1Trans.position, CameraMovetoPlayer);
		CamTrans.position.z=-10;
		CamCamera.orthographicSize=Mathf.MoveTowards(CamCamera.orthographicSize,90,CameraZoomtoPlayer);
	}
	else if(bullet2.active==true&&GameSystemFkt.bulletIsFlying==false){
		//CamTrans.position.x=bullet2Trans.position.x+OffsetCameraX;
		CamTrans.position = Vector3.MoveTowards(CamTrans.position,bullet2Trans.position, CameraMovetoPlayer);
		CamTrans.position.z=-10;
		CamCamera.orthographicSize=Mathf.MoveTowards(CamCamera.orthographicSize,90,CameraZoomtoPlayer);
	}
	else if(bullet3.active==true&&GameSystemFkt.bulletIsFlying==false){
		CamTrans.position = Vector3.MoveTowards(CamTrans.position,bullet3Trans.position, CameraMovetoPlayer);
		CamTrans.position.z=-10;
		CamCamera.orthographicSize=Mathf.MoveTowards(CamCamera.orthographicSize,90,CameraZoomtoPlayer);
	}
	else if(bullet4.active==true&&GameSystemFkt.bulletIsFlying==false){
		CamTrans.position = Vector3.MoveTowards(CamTrans.position,bullet4Trans.position, CameraMovetoPlayer);
		CamTrans.position.z=-10;
		CamCamera.orthographicSize=Mathf.MoveTowards(CamCamera.orthographicSize,90,CameraZoomtoPlayer);
	}
	else if(bullet5.active==true&&GameSystemFkt.bulletIsFlying==false){
		CamTrans.position = Vector3.MoveTowards(CamTrans.position,bullet5Trans.position, CameraMovetoPlayer);
		CamTrans.position.z=-10;
		CamCamera.orthographicSize=Mathf.MoveTowards(CamCamera.orthographicSize,90,CameraZoomtoPlayer);
	}
	else if(bullet6.active==true&&GameSystemFkt.bulletIsFlying==false){
		CamTrans.position = Vector3.MoveTowards(CamTrans.position,bullet6Trans.position, CameraMovetoPlayer);
		CamTrans.position.z=-10;
		CamCamera.orthographicSize=Mathf.MoveTowards(CamCamera.orthographicSize,90,CameraZoomtoPlayer);
	}
	if(GameSystemFkt.bulletIsFlying==true){
		CamCamera.orthographicSize=Mathf.MoveTowards(CamCamera.orthographicSize,150,CameraMoveAwayPlayer);
		CamTrans.position = Vector3.MoveTowards(CamTrans.position,Vector3(0,0,-10), CameraZoomAwayPlayer);
	}
